...
io.on('connection', (socket) => {
     ...
     */
    socket.on('messageDetection', (chat, message, token) => {
        console.log(findRooms());
        console.log("LLEGA: " + actualRoom);
        // Parse String To Json
        let chatParsed = JSON.parse(chat);
        let newMessage = new Message_1.Message(chatParsed.id, chatParsed.matchUser1, chatParsed.matchUser2, message);
 ...
 * @param token
 */
let addMessage = (message, token, room) => {
    let url = `${constants_1.SERVER}/message/add`;
    fetch(url, {
        method: 'POST',
        headers: {
            'Authorization': token,
